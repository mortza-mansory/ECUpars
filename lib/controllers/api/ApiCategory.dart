...
class CategoryController extends GetxController {
  ...
  void navigateToCategoryDetails(int categoryId, {bool isRoute = false}) {
    ...
    loadCategoryDetails(categoryId).then((success) {
      if (success) {
        Get.to(() => GenericCategoryScreen(categoryId: categoryId, isRoot: rootCategoryId == categoryId));
      } else {
        // Get.snackbar(
        //   "Error".tr,
    ...
    if (navigationStack.isNotEmpty) {
      ...
      if (navigationStack.isNotEmpty) {
        ...
        loadCategoryDetails(previousCategoryId).then((success) {
          if (success) {
            Get.off(() => GenericCategoryScreen(categoryId: previousCategoryId, isRoot: false));
          } else {
            // Get.snackbar(
            //   "Error".tr,
